    ...
    public class SlaDefinitionRepository : ISlaDefinitionRepository
    {
        ...
        public async Task SoftDeleteAsync(Guid id, CancellationToken cancellationToken = default)
        {
            ...
            if (entity is not null)
            {
                ...
                // Delete associated reports so charts/tables stay clean
                var reports = await _context.SlaReports
                    .Where(r => r.SlaDefinitionId == id)
                    .ToListAsync(cancellationToken);
                _context.SlaReports.RemoveRange(reports);
