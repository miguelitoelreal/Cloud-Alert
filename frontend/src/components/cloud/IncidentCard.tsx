...
export function IncidentCard({ variant, incident, isNew, onSelect, dense }: IncidentCardProps & { dense?: boolean }) {
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-1 text-[10px] text-slate-400">
            <span className="font-mono text-slate-300">{new Date(incident.occurredAt).toLocaleTimeString("es-PE", { hour: "2-digit", minute: "2-digit", timeZone: "America/Lima" })}</span>
            <span>·</span>
      ...
      <h3 className={`font-semibold text-slate-100 ${dense ? "mt-1.5 text-sm" : "mt-3 text-base"}`}>{incident.title}</h3>
      <div className={`flex items-center gap-1.5 font-mono text-emerald-400 ${dense ? "mt-0.5 text-[10px]" : "mt-1 text-xs"}`}>
      ...
      <p className={`line-clamp-2 text-slate-400 ${dense ? "mt-1 text-xs leading-4" : "mt-1.5 text-sm leading-5"}`}>{incident.description}</p>

      <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px] text-slate-500">
        {incident.region ? (
          <span className="inline-flex items-center gap-1 rounded bg-slate-800/60 px-1.5 py-0.5 text-slate-300">
        ...
        <span className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 ${incident.isActive ? "bg-emerald-900/20 text-emerald-300" : "bg-slate-800/60 text-slate-300"}`}>
          <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          {incident.isActive ? "Activo" : "Inactivo"}
        </span>
        <span className="inline-flex items-center gap-1 rounded bg-slate-800/60 px-1.5 py-0.5 text-slate-300" title={formatDateTime(incident.occurredAt)}>
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        {incident.affectedServices.length > 0 ? (
          incident.affectedServices.map((service) => (
            <span key={service} className="rounded-full bg-slate-800 px-2 py-0.5 text-[10px] font-medium text-slate-300">
              {service}
            </span>
          ))
        ) : (
      ...
      <div className={`flex flex-wrap items-center justify-between gap-2 border-t border-slate-800 text-xs ${dense ? "mt-2 pt-2" : "mt-3 pt-3"}`}>
        <div className="flex items-center gap-1.5">
          {incident.resolvedAt ? (
            <>
              <span className="inline-flex items-center gap-1 rounded bg-emerald-900/30 px-1.5 py-0.5 text-emerald-300 font-medium">
              ...
              <span className="font-mono text-slate-300">{new Date(incident.resolvedAt).toLocaleTimeString("es-PE", { hour: "2-digit", minute: "2-digit", timeZone: "America/Lima" })}</span>
              <span className="text-slate-500">({relativeTime(incident.resolvedAt)})</span>
            </>
          ) : (
            <>
              <span className="inline-flex items-center gap-1 rounded bg-amber-900/30 px-1.5 py-0.5 text-amber-300 font-medium animate-pulse">
                <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" /></svg>
                En seguimiento
              </span>
              <span className="font-mono text-slate-300">{new Date(incident.occurredAt).toLocaleTimeString("es-PE", { hour: "2-digit", minute: "2-digit", timeZone: "America/Lima" })}</span>
              <span className="text-slate-500">({relativeTime(incident.occurredAt)})</span>
            </>
          )}
        </div>
        <div className="flex items-center gap-1.5 text-blue-600 hover:text-blue-300">Ver detalles →</div>
      </div>
    </article>
  );
}
