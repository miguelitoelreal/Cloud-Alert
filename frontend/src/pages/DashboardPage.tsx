...
function mergeMonitorUpdate(
  ...
  return {
    ...
    lastDnsTimeMs: update.lastDnsTimeMs ?? current?.lastDnsTimeMs ?? null,
    lastConnectTimeMs: update.lastConnectTimeMs ?? current?.lastConnectTimeMs ?? null,
    lastTlsTimeMs: update.lastTlsTimeMs ?? current?.lastTlsTimeMs ?? null,
    lastTtfbTimeMs: update.lastTtfbTimeMs ?? current?.lastTtfbTimeMs ?? null,
    uptimePercentage:
...
export function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
            ...
            <span
              className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${
                isRealtimeConnected
                  ...
                  : "border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 animate-pulse"
              }`}
            >
              <span
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="primary" onClick={openCreate}>
              <span className="flex items-center gap-1.5">
                ...
                <span className="hidden sm:inline">Nuevo monitor</span>
      ...
      >
        {isLoading ? (
          <div className="flex items-center gap-3 py-8 text-sm text-slate-500 dark:text-slate-400">
            <div className="h-4 w-4 animate-spin rounded-full border-2 border-slate-300 dark:border-slate-600 border-t-blue-400" />
            Cargando monitores…
          </div>
        ) : monitors.length === 0 ? (
          <div className="py-10 text-center">
            <div className="mx-auto inline-flex rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900/50 p-4">
          ...
          <>
            <div className="hidden md:block overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                <thead className="border-b border-slate-300 dark:border-slate-700 text-xs font-semibold uppercase tracking-wider text-slate-500">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800">
                  {monitors.map((m) => (
                    ...
                    >
                      ...
                      <td
                        className="px-3 py-3 text-slate-500 dark:text-slate-400 cursor-help"
                        title={
                          m.lastDnsTimeMs != null || m.lastConnectTimeMs != null || m.lastTlsTimeMs != null || m.lastTtfbTimeMs != null
                            ? `DNS: ${m.lastDnsTimeMs ?? "—"}ms | Conexión: ${m.lastConnectTimeMs ?? "—"}ms | TLS: ${m.lastTlsTimeMs ?? "—"}ms | TTFB: ${m.lastTtfbTimeMs ?? "—"}ms`
                            : undefined
                        }
                      >
                      ...
                      <td className="px-3 py-3 text-slate-700 dark:text-slate-300">{m.totalChecks}</td>
                      <td className="px-3 py-3">
                        <span
                          className={
                            (m.failedChecks ?? 0) > 0
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-2 opacity-80 transition-opacity group-hover:opacity-100">
                          ...
                          <button
                            type="button"
                            className="rounded-lg px-2.5 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 transition-colors hover:bg-slate-700 hover:text-white"
            ...
            <div className="md:hidden space-y-3">
              {monitors.map((m) => (
                <div
                  key={m.id}
                  className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 p-4 space-y-3"
                >
                  <div className="flex items-center justify-between gap-2">
                    <Link
                      to={`/monitors/${m.id}`}
                      className="min-w-0 text-sm font-semibold text-white transition-colors hover:text-blue-400 truncate"
                    >
                      {m.name}
                    </Link>
                    <StatusBadge status={m.currentStatus} />
                  </div>
                  <div className="text-xs text-slate-500 dark:text-slate-400 truncate" title={m.url}>
                    {m.url}
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      <span className="text-slate-500">Verificación:</span>{" "}
                      <span className="text-slate-300">{m.lastCheckedAt ? formatDateTime(m.lastCheckedAt) : "—"}</span>
                    </div>
                    <div>
                      <span className="text-slate-500">Respuesta:</span>{" "}
                      <span className="text-slate-300">{m.lastResponseTimeMs != null ? `${m.lastResponseTimeMs} ms` : "—"}</span>
                    </div>
                    <div>
                      <span className="text-slate-500">Uptime:</span>{" "}
                      <span className="text-slate-300">{m.uptimePercentage != null ? `${m.uptimePercentage.toFixed(1)}%` : "—"}</span>
                    </div>
                    <div>
                      <span className="text-slate-500">Checks:</span>{" "}
                      <span className="text-slate-300">{m.totalChecks}</span>
                      {((m.failedChecks ?? 0) > 0) && (
                        <span className="ml-1 text-rose-400">({m.failedChecks} fallos)</span>
                      )}
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 pt-1">
                    <button
                      type="button"
                      className="rounded-lg border border-slate-200 dark:border-slate-700 px-2.5 py-1.5 text-xs font-medium text-blue-400 transition-colors hover:bg-blue-500/10"
                      onClick={() => void openNetInfo(m)}
                    >
                      WHOIS / SSL
                    </button>
                    <button
                      type="button"
                      className="rounded-lg border border-slate-200 dark:border-slate-700 px-2.5 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 transition-colors hover:bg-slate-700 hover:text-white"
                      onClick={() => void openEdit(m.id)}
                    >
                      Editar
                    </button>
                    <button
                      type="button"
                      className="rounded-lg border border-slate-200 dark:border-slate-700 px-2.5 py-1.5 text-xs font-medium text-rose-400 transition-colors hover:bg-rose-500/10"
                      onClick={() => openDelete(m)}
                    >
                      Eliminar
                    </button>
      ...
      >
        {netInfoLoading ? (
          <div className="flex items-center gap-3 py-6 text-sm text-slate-500 dark:text-slate-400">
            <div className="h-4 w-4 animate-spin rounded-full border-2 border-slate-300 dark:border-slate-600 border-t-blue-400" />
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-end">
              <button
                ...
                className="flex items-center gap-1.5 rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 px-3 py-1.5 text-xs font-medium text-slate-700 dark:text-slate-300 transition-colors hover:bg-slate-100 dark:bg-slate-800"
              >
            ...
            {netInfoData.whois && (
              <div>
                <h4 className="mb-3 flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-slate-700 dark:text-slate-300">
