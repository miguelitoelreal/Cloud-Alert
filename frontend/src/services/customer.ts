...
export async function importCustomers(payload: { customers: CreateCustomerPayload[] }): Promise<{ importedCount: number; failedCount: number; errors: string[] }> {
  const response = await apiClient.post<{ importedCount: number; failedCount: number; errors: string[] }>('/api/customers/bulk', payload);
  return response.data;
}
